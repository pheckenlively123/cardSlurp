...
func TestIsFileSame(t *testing.T) {
	...
	for transBuff := 1; transBuff <= maxTransBuff; transBuff++ {
		...
		sameStat, err := IsFileSame("testData/same_a.txt", "testData/same_b.txt", uint64(transBuff))
		if err != nil {
		...
		diffStat, err := IsFileSame("testData/same_a.txt", "testData/diff_b.txt", uint64(transBuff))
		if err != nil {
			fmt.Print("Error calling IsFileSame: " + err.Error() + "\n")
		}
...
func TestNibbleCopy(t *testing.T) {
	...
	for transBuff := 1; transBuff <= maxTransBuff; transBuff++ {
		...
		nibStat, err := NibbleCopy("testData/same_a.txt", "testData/victim.txt", uint64(transBuff))
		if err != nil {
		...
		sameStat, err := IsFileSame("testData/same_a.txt", "testData/victim.txt", uint64(transBuff))
		if err != nil {
